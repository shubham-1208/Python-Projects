...
def CreateBoard(num_rows, num_cols, num_cells):
    ...
    for i in range(conf_puzzle.RAND_NUM):
...
def ShowEndInterface(screen, width, height):
    screen.fill(conf_puzzle.BACKGROUND_COLOR)
    font = pygame.font.Font(conf_puzzle.FONTPATH, width//15)
...
def ShowStartInterface(screen, width, height):
    screen.fill(conf_puzzle.BACKGROUND_COLOR)
    tfont = pygame.font.Font(conf_puzzle.FONTPATH, width//4)
    cfont = pygame.font.Font(conf_puzzle.FONTPATH, width//20)
    title = tfont.render('PUZZLE', True, conf_puzzle.RED)
    content1 = cfont.render('Press H, M or L to choose your puzzle', True, conf_puzzle.BLUE)
    content2 = cfont.render('H- 5*5, M- 4*4, L- 3*3', True, conf_puzzle.BLUE)
...
def main():
    ...
    game_img_used = pygame.image.load(GetImagePath(conf_puzzle.PICTURE_ROOT_DIR))
    game_img_used = pygame.transform.scale(game_img_used, conf_puzzle.SCREENSIZE)
    game_img_used_rect = game_img_used.get_rect()
 
    screen = pygame.display.set_mode(conf_puzzle.SCREENSIZE)
    ...
    while is_running:
        ...
        for i in range(num_cols+1):
            pygame.draw.line(screen, conf_puzzle.BLACK, (i*cell_width, 0), (i*cell_width, game_img_used_rect.height))
        for i in range(num_rows+1):
            pygame.draw.line(screen, conf_puzzle.BLACK, (0, i*cell_height), (game_img_used_rect.width, i*cell_height))
        pygame.display.update()
        clock.tick(conf_puzzle.FPS)
